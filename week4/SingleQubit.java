public class SingleQubit extends ParentQubit {
    ...
    public DoubleQubit mergeQubits(ParentQubit pq) {
        ...
        System.out.println(TestUtils.arrToStr(this.values));
        System.out.println(TestUtils.arrToStr(vals));

        float[] allVals = MatrixMath.tensorProd(this.values, vals);
        System.out.println(TestUtils.arrToStr(allVals));
        ...
        for (int i = 0; i < allVals.length; i++) {
            ...
            newVals[i] = phase * (float) Math.pow(allVals[i], 2);
        }
